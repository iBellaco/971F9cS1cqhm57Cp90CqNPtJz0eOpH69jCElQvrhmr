...
# We need to parse each item block.
...
for line in content.split('\n'):
    ...
    # Passive can span multiple lines or have escaped quotes, but in this file it's usually on one line with \n escapes.
...
for item in items:
    print(f"### {item.get('name', 'N/A')} <img src='{item.get('iconUrl', '')}' width='32' align='center'/>")
    print(f"- **Costo:** {item.get('goldCost', 'N/A')} 🟡")
    print(f"- **Estadísticas:** {item.get('stats', 'Ninguna')}")
    # clean up passive string a bit
    passive = item.get('passive', '').replace('\\n', ' | ')
    print(f"- **Efecto / Pasiva:** {passive}")
    print()
