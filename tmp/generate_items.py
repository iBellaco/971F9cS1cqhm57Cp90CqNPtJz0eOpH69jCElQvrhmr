...
total_count = 0
for cat_title, cat_enum, section_header, exp_count in categories_map:
    h_elem = None
    for h in soup.find_all(['h2', 'h3']):
        if cat_title.lower() in h.get_text().lower():
            h_elem = h
            break
    if not h_elem:
        continue
    container = h_elem.find_next_sibling('div', class_='items-list-container')
    if not container:
        container = h_elem.find_next('div', class_='items-list-container')
...
'''
    if container:
        for div in container.find_all('div', class_='item'):
            ...
            t_soup = BeautifulSoup(title_attr, 'html.parser')
            
            name = t_soup.find('div', class_='item-tooltip-title')
            raw_name = name.get_text(strip=True) if name else ''
            
            cost = t_soup.find('div', class_='item-tooltip-cost')
            cost_val = int(re.sub(r'[^\d]', '', cost.get_text(strip=True))) if cost and re.sub(r'[^\d]', '', cost.get_text(strip=True)) else 0
            
            stats = t_soup.find('div', class_='item-tooltip-stats')
            stats_str = ', '.join([s.strip() for s in stats.stripped_strings]) if stats else ''
            
            passives = t_soup.find('div', class_='item-tooltip-passives')
            passives_str = ' '.join([p.get_text(' ', strip=True) for p in passives.find_all('div', class_='item-tooltip-passives-block')]) if passives else ''
            ...
            if not raw_name and img:
                raw_name = img.get('alt', '')
...
print(f'Successfully wrote {total_count} items to /app/src/main/java/com/example/data/WildRiftItemsData.kt')
