...
# Replace from `val allySlots = arrayOfNulls...` up to `// 2. Identificar el slot del jugador local`
pattern = re.compile(r'val allySlots = arrayOfNulls<Champion>\(5\).*?// 2\. Identificar el slot del jugador local', re.DOTALL)
...
new_code = r"""val allySlots = arrayOfNulls<Champion>(5)
            ...
            // Colecciones dinámicas para evitar solapamientos por aspecto de pantalla
            ...
            for (block in visionText.textBlocks) {
                for (line in block.lines) {
                    ...
                    if (xRatio in 0.06f..0.33f) {
                        ...
                        allySlotTexts[bucket].add(lineText) // Para detección de jugador local
                    ...
                    // --- COLUMNA DERECHA: EQUIPO ENEMIGO ---
                        ...
                        if (candidate != null && detectedEnemyChamps.none { it.first.id == candidate.id } && detectedAllyChamps.none { it.first.id == candidate.id }) {
                            ...
                            AppLogger.d(TAG, "Enemigo detectado en Y=$yRatio: ${candidate.name}")
                        }
                    }
                }
            }

            // Ordenar por Y-ratio para mantener el orden visual real de la pantalla
            ...
            // Asignar secuencialmente a los slots (0 a 4) según el orden visual
            ...
            // 2. Identificar el slot del jugador local"""
