...
# All 52 Runes with exact EN and PT translations
runes_translations = [
    {
        "id": "dark_harvest",
...
# Spells with exact EN and PT translations
spells_translations = [
    {
        "id": "ghost",
        "name_es": "Fantasmal",
...
print(f"Total runes configured: {len(runes_translations)}")
print(f"Total spells configured: {len(spells_translations)}")

# Save to a json file for ingestion
with open('/tmp/wr_translations_ready.json', 'w', encoding='utf-8') as f:
    json.dump({
        "runes": runes_translations,
        "spells": spells_translations
    }, f, ensure_ascii=False, indent=2)

print("Saved /tmp/wr_translations_ready.json")
