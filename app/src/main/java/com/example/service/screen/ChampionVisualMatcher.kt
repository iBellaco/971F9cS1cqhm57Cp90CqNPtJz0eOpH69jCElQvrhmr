...
object ChampionVisualMatcher {
    ...
    data class VisualSignature(
        ...
        val spatialRgb: FloatArray // 256 * 3 valores RGB normalizados (0.0..1.0)
    )
    ...
    /**
     * Calcula la firma visual (histograma + matriz espacial) de una imagen.
     */
    fun computeSignature(bitmap: Bitmap, isCroppedAvatar: Boolean): VisualSignature? {
        ...
        // Si es un recorte de avatar de pantalla, tomar el 75% central para evitar
        // bordes dorados, azules, rojos o sombras del marco de la interfaz.
        val startX = if (isCroppedAvatar) (w * 0.12f).toInt() else 0
        val endX = if (isCroppedAvatar) (w * 0.88f).toInt() else w
        val startY = if (isCroppedAvatar) (h * 0.12f).toInt() else 0
        val endY = if (isCroppedAvatar) (h * 0.88f).toInt() else h
        ...
        // Redimensionar a 16x16 para análisis estandarizado
        val scaled = if (workingBmp.width == 16 && workingBmp.height == 16) {
            workingBmp
        } else {
            Bitmap.createScaledBitmap(workingBmp, 16, 16, true)
        }
        ...
        // 1. Histograma 4x4x4 = 64 bins
        val hist = FloatArray(64)
        val spatial = FloatArray(256 * 3)
        ...
        for (i in 0 until 256) {
            ...
            // Matriz espacial normalizada 0..1
            spatial[i * 3] = r / 255.0f
            spatial[i * 3 + 1] = g / 255.0f
            spatial[i * 3 + 2] = b / 255.0f
        }

        // Normalizar histograma a suma 1.0
        for (k in 0 until 64) {
            hist[k] = hist[k] / 256.0f
        }

        return VisualSignature(hist, spatial)
    }
    ...
    fun matchChampion(
        ...
        for (champ in eligibleCandidates) {
            ...
            // 1. Similaridad de Histograma (Intersección de histogramas normalizados)
            var histSim = 0.0f
            for (i in 0 until 64) {
                histSim += min(targetSignature.histogram[i], candidateSig.histogram[i])
            }

            // 2. Similaridad Espacial (Diferencia euclidiana normalizada)
            var spatialDiffSum = 0.0f
            for (i in 0 until 256) {
                val dr = targetSignature.spatialRgb[i * 3] - candidateSig.spatialRgb[i * 3]
                val dg = targetSignature.spatialRgb[i * 3 + 1] - candidateSig.spatialRgb[i * 3 + 1]
                val db = targetSignature.spatialRgb[i * 3 + 2] - candidateSig.spatialRgb[i * 3 + 2]
                spatialDiffSum += sqrt((dr * dr + dg * dg + db * db) / 3.0f)
            }
            val avgSpatialDiff = spatialDiffSum / 256.0f
            val spatialSim = (1.0f - avgSpatialDiff).coerceIn(0.0f, 1.0f)

            // Puntuación combinada ponderada
            val combinedScore = 0.40f * histSim + 0.60f * spatialSim
