...
# Now replace lines 268 to 407 in MetaAndDraftScreen.kt with replacement.txt
awk '
NR==268 {
    while ((getline line < "replacement.txt") > 0) {
        print line
    }
    close("replacement.txt")
    skip=1
}
NR==408 {
    skip=0
...
!skip { print }
' app/src/main/java/com/example/ui/screens/MetaAndDraftScreen.kt > temp.kt && mv temp.kt app/src/main/java/com/example/ui/screens/MetaAndDraftScreen.kt
