...
# Let's find where the list of WildRiftItem objects ends.
# Searching for the last ")," before the end of the list assignment or something.
# We can just insert it right before the last closing bracket of the list.
# The list declaration is: val list: List<WildRiftItem> = listOf( ... )
# Let's just append right after the last WildRiftItem, or replace the last `        )` with `        ),` and append our new string and a closing `    )`

# Best way: find `    )` at the end of the file.
# The file ends with:
#         )
#     )
# }
# So we can do a regex sub or just a string replace on the last `    )`

import re
# Replace the last `\n    )` with `,\n` + new_items_code + `\n    )`
# wait, actually the end of listOf is `    )`
last_parenthesis = content.rfind('    )')
if last_parenthesis != -1:
    new_content = content[:last_parenthesis] + new_items_code + content[last_parenthesis:]
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Items injected successfully.")
else:
    print("Could not find the end of listOf.")
