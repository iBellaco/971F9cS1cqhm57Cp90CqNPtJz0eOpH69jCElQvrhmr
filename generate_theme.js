const fs = require('fs');
const old = fs.readFileSync('app/src/main/java/com/example/ui/components/ThemeCustomizationDialog.kt', 'utf8');
const header = old.substring(0, old.indexOf('@OptIn(ExperimentalMaterial3Api::class)'));
...
const regionCardCode = old.substring(old.indexOf('@Composable\nprivate fun RegionVisualPreviewGridCard('));

let finalRegion = regionCardCode;
if (finalRegion.indexOf('@Composable\nprivate fun NavBarCustomizationTab') !== -1) {
    finalRegion = finalRegion.substring(0, finalRegion.indexOf('@Composable\nprivate fun NavBarCustomizationTab'));
}

fs.writeFileSync('app/src/main/java/com/example/ui/components/ThemeCustomizationDialog.kt', header + body + "\n" + finalRegion);
console.log("Written!");
