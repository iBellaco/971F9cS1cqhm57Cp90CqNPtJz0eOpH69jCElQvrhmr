...
# Replace the Column in FloatingDraftCoachView
pattern = re.compile(
    r'    Column\(\s*modifier = Modifier\s*\.fillMaxSize\(\)\s*\.verticalScroll\(rememberScrollState\(\)\)\s*\) \{\s*// 1\. TABLERO DE DRAFT.*?Spacer\(modifier = Modifier\.height\(12\.dp\)\)',
    re.DOTALL
)

new_content = """    val configuration = androidx.compose.ui.platform.LocalConfiguration.current
