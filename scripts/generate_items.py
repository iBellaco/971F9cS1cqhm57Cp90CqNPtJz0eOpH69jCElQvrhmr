...
# Pattern for section titles: <h[23][^>]*>(.*?)</h[23]>
sections = re.findall(r'<h[23][^>]*>(.*?)</h[23]>([\s\S]*?)(?=<h[23]|$)', raw_html)
...
for cat_title, cat_enum, section_header, exp_count in categories_spec:
    section_content = ""
    for title_text, body in sections:
        if cat_title.lower() in title_text.lower():
    ...
    kotlin_code += f'''        // =========================================================================
        ...
        // =========================================================================
'''
    # Find all <div class="item"...
    item_blocks = re.findall(r'<div class="item"[^>]*title="([^"]*)"[^>]*>([\s\S]*?)</div>\s*(?=<div class="item"|</div>\s*</div>|$)', section_content)
    
    cat_items_count = 0
    for title_attr, inner_html in item_blocks:
        unescaped_title = html.unescape(title_attr)
        
        # Name
        name_match = re.search(r"class='item-tooltip-title'>(.*?)</div>", unescaped_title)
        raw_name = strip_tags(name_match.group(1)) if name_match else ""
        
        # Cost
        cost_match = re.search(r"class='item-tooltip-cost'>(.*?)</div>", unescaped_title)
        cost_str = re.sub(r'[^\d]', '', cost_match.group(1)) if cost_match else "0"
        cost_val = int(cost_str) if cost_str else 0
        
        # Stats
        stats_match = re.search(r"class='item-tooltip-stats'>(.*?)</div>", unescaped_title)
        ...
        # Image
        img_match = re.search(r'src="([^"]+)"', inner_html) or re.search(r'data-src="([^"]+)"', inner_html)
