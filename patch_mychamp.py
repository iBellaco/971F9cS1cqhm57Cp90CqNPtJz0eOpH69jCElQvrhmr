...
# 1. Tabs removal
content = content.replace('tr("Objetivos"),\n                tr("⏱️ CD Tracker"),\n                tr("🛡️ Math Daño")', 'tr("Objetivos")')

# 2. Case 6, 7, 8 removal
case_target = """                    6 -> {
...
# 3. Add myChampion state
content = content.replace('var pickingForTeam by remember { mutableStateOf<String?>(null) } // "ALLY", "ENEMY"', 'var pickingForTeam by remember { mutableStateOf<String?>(null) } // "ALLY", "ENEMY", "MYSELF"\n    var myChampion by remember { mutableStateOf<Champion?>(null) }')

# 4. DraftAnalysisTab call modification
content = content.replace('DraftAnalysisTab(\n                            activeRole = activeRole,\n                            allies = allyChampions,\n                            enemies = enemyChampions,\n                            analysis = analysis,\n                            isFirstPick = isFirstPick,\n                            onToggleFirstPick = { isFirstPick = !isFirstPick },\n                            onChangeRole = { showRoleChangeDialog = true },\n                            onAddAlly = { pickingForTeam = "ALLY" },', 'DraftAnalysisTab(\n                            myChampion = myChampion,\n                            activeRole = activeRole,\n                            allies = allyChampions,\n                            enemies = enemyChampions,\n                            analysis = analysis,\n                            isFirstPick = isFirstPick,\n                            onToggleFirstPick = { isFirstPick = !isFirstPick },\n                            onChangeRole = { showRoleChangeDialog = true },\n                            onAddMyChampion = { pickingForTeam = "MYSELF" },\n                            onRemoveMyChampion = { myChampion = null },\n                            onAddAlly = { pickingForTeam = "ALLY" },')

# 5. DraftChampionPickerSheet modification
picker_target = """        DraftChampionPickerSheet(
            team = pickingForTeam ?: "",
...
picker_replacement = """        DraftChampionPickerSheet(
            team = pickingForTeam ?: "",
            alreadySelected = (allyChampions + enemyChampions).map { it.id } + (myChampion?.id?.let { listOf(it) } ?: emptyList()),
            onChampionPicked = { champ ->
                if (pickingForTeam == "MYSELF") {
                    myChampion = champ
                } else if (pickingForTeam == "ALLY" && allyChampions.size < 5) {
                    allyChampions.add(champ)
                } else if (pickingForTeam == "ENEMY" && enemyChampions.size < 5) {
                    enemyChampions.add(champ)
                }
...
content = content.replace(picker_target, picker_replacement)

# 6. DraftAnalysisTab definition modification
content = content.replace('private fun DraftAnalysisTab(\n    activeRole: LaneRole,\n    allies: List<Champion>,\n    enemies: List<Champion>,\n    analysis: DraftAnalysisResult,\n    isFirstPick: Boolean,\n    onToggleFirstPick: () -> Unit,\n    onChangeRole: () -> Unit,\n    onAddAlly: () -> Unit,', 'private fun DraftAnalysisTab(\n    myChampion: Champion?,\n    activeRole: LaneRole,\n    allies: List<Champion>,\n    enemies: List<Champion>,\n    analysis: DraftAnalysisResult,\n    isFirstPick: Boolean,\n    onToggleFirstPick: () -> Unit,\n    onChangeRole: () -> Unit,\n    onAddMyChampion: () -> Unit,\n    onRemoveMyChampion: () -> Unit,\n    onAddAlly: () -> Unit,')

# 7. Evaluation logic replacement
